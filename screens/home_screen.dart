...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _refreshGroup(BuildContext context, AppState state, VpnGroup g) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(err == null ? '${g.title} обновлена' : err),
...
class _GroupHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!isUnlimited && fraction != null) {
      if (fraction > 0.9) barColor = c.red;
...
class _ServerTileState extends State<_ServerTile> with SingleTickerProviderStateMixin {
  ...
  static const double _deadZone = 20;  
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          IosListTile(
            ...
            onTap: () {
              ...
              if (g != null) Navigator.of(context).push(MaterialPageRoute(
  ...
  void _showServerInfo(BuildContext context) {
    ...
    final inboundPort = node.params['inbound_port'] ?? node.port;
    ...
    const _excludedKeys = ['type', 'network', 'security', 'sni', 'peer',
                           'fp', 'fingerprint', 'flow', 'alpn', 'pbk',
                           'sid', 'inbound_port'];
    final extraParams = Map<String, dynamic>.fromEntries(
      node.params.entries.where((e) => !_excludedKeys.contains(e.key)),
    );
