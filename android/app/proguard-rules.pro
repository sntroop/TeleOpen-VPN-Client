...
# ── ciadpi (ByeDPI) JNI ──────────────────────────────────────────────
# Нативный код зовёт protect(I)Z по строковому имени через JNI —
# R8 не должен переименовывать ни нативные методы, ни SocketProtector.
-keep class space.teleopen.app.CiadpiNative { *; }
-keep class space.teleopen.app.CiadpiNative$SocketProtector { *; }
-keepclasseswithmembernames class space.teleopen.app.CiadpiNative {
    native <methods>;
}
# Сохранить имя метода protect(int) на ЛЮБОЙ реализации интерфейса
# (в т.ч. на анонимном HysteriaTunVpnService$a из startByeDpi).
-keep class * implements space.teleopen.app.CiadpiNative$SocketProtector { *; }
-keepclassmembers class * implements space.teleopen.app.CiadpiNative$SocketProtector {
    boolean protect(int);
}
