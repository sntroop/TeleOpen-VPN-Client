...
`v1.0.3 (5001)` · Android (primary) · iOS/desktop targets present
...
> ⚠️ Это **рабочий каталог разработки** (`my_vpn`). Публичное зеркало кода со
> скриншотами и нативными исходниками живёт в отдельном репозитории
> `TeleOpen-VPN-Client` и обновляется вручную — следи за рассинхроном.
...
- **Натив**: общение через `MethodChannel`/`EventChannel`
  (`com.example.my_vpn/native`, `.../vpn_status`) — см. `lib/vpn_bridge.dart`.
...
См. `LICENSE` в публичном репозитории (`AGPL-3.0`).
