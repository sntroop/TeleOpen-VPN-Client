...
class Hysteria2Manager {
  ...
  /// Собирает hysteria2-конфиг (map) из hy2://-URI. Вынесено отдельно, чтобы
  /// тем же кодом готовить и рантайм-запуск (start), и «профиль запуска» для
  /// нативного виджета/тайла (WidgetBridge), который натив поднимает сам.
  /// Возвращает null, если URI нераспарсиваем.
  static Map<String, dynamic>? buildConfig(String hy2Uri) {
  ...
  static Future<bool> start(String hy2Uri) async {
    ...
    try {
      ...
      final hy2Config = buildConfig(hy2Uri);
      if (hy2Config == null) return false;
      ...
      lastServer = (hy2Config['server'] as String?) ?? '';
