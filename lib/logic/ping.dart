// lib/logic/ping.dart
//
// Пинг нод. Поддерживает четыре режима (AppSettings.pingMode):
...
//   GET  — делаем прямой HTTP(S) GET до host:port и меряем полный ответ;
...
class TcpPing {
  ...
  static Future<int?> ping(String host, int port,
  ...
  /// HTTP(S) GET до host:port. Это прямой запрос к endpoint'у ноды, без VPN-ядра.
  static Future<int?> pingGet(
    String host,
    int port, {
    Duration timeout = const Duration(seconds: 5),
  }) async {
    final scheme = _httpsPorts.contains(port) ? 'https' : 'http';
    final uri = Uri(scheme: scheme, host: host, port: port, path: '/');
    ...
    try {
      ...
      req.headers.set(HttpHeaders.userAgentHeader, 'TeleOpen-Ping/1.0');
  ...
  /// [mode] выбирает транспорт зонда: TCP, UDP или GET.
    ...
    Future<void> worker() async {
      while (true) {
        ...
        final ms = switch (mode) {
          'UDP' => await pingUdp(t.host, t.port),
          'GET' => await pingGet(t.host, t.port),
          _ => await ping(t.host, t.port),
        };
        onResult(i, ms);
      }
    }
