...
/// Возвращает xray-конфиг для запуска/измерения [node].
///
/// Если у ноды есть исходный полный JSON-конфиг из подписки ([VpnNode.rawConfig])
/// — отдаём ЕГО (вычистив слушающие inbounds), чтобы не терять транспортные поля
/// (grpc serviceName, tls alpn, xhttp, spiderX…), которые искажались при
/// пересборке в vless://. Иначе — собираем конфиг из params (share-ссылки).
///
/// Hysteria2 здесь по-прежнему не обрабатывается (для него отдельный движок) —
/// вызывающий код исключает её до вызова, как и раньше для buildXrayConfig.
String xrayConfigForNode(VpnNode node,
    {bool packetSniffing = true,
    bool useMux = false,
    bool resolveDestination = false}) {
  final raw = node.rawConfig;
  if (raw != null && raw.isNotEmpty) {
    return prepareRawXrayConfig(raw);
  }
  return buildXrayConfig(node,
      packetSniffing: packetSniffing,
      useMux: useMux,
      resolveDestination: resolveDestination);
}

/// Готовит сырой JSON-конфиг подписки к запуску в VPN-режиме: убирает
/// слушающие inbounds (socks/http на 127.0.0.1, которые кладут панели). В
/// VPN-режиме весь трафик идёт через TUN-inbound — его дописывает натив
/// (HysteriaTunVpnService.ensureTunInbound). Лишний слушающий порт при
/// реконнекте даёт нативный краш 'bind: address already in use'. Outbounds,
/// dns и routing из конфига сервера сохраняем как есть.
String prepareRawXrayConfig(String raw) {
  try {
    final root = jsonDecode(raw) as Map<String, dynamic>;
    root['inbounds'] = <dynamic>[];
    return jsonEncode(root);
  } catch (_) {
    // Не разобрался как JSON — отдаём как есть, натив разрулит/откатится.
    return raw;
  }
}
