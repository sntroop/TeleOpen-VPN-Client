...
///
/// Поддерживает «обёрнутые» ссылки (ssconnect.app/link?url_ha=happ%3A%2F%2F…):
/// happ:// извлекается из percent-кодировки до расшифровки.
Future<String> expandHappLinks(String raw) async {
  // Сначала разворачиваем возможную обёртку: одна ссылка-редиректор на строке.
  final unwrapped = raw
      .split('\n')
      .map((l) => unwrapHappLink(l))
      .join('\n');
  if (!unwrapped.contains('happ://')) return raw;
  final keys = await loadHappKeys();
  final out = <String>[];
  for (final line in unwrapped.split('\n')) {
    final t = line.trim();
    if (isHappLink(t)) {
