...
class UpdaterService extends ChangeNotifier {
  ...
  Future<void> downloadAndInstall() async {
    ...
    try {
      ...
      // Качаем с устойчивостью к обрыву: при «Connection closed while
      // receiving data» (флаки мобильный канал на 100-МБ файле) дописываем
      // хвост через Range вместо того чтобы стартовать с нуля.
      await _downloadWithResume(file, info);
  ...
  /// Скачивание APK с устойчивостью к обрывам соединения.
  ///
  /// На 100-МБ файле и нестабильном мобильном канале одиночный GET часто
  /// падает посреди стрима ("Connection closed while receiving data"). Здесь
  /// мы при каждом обрыве дописываем недостающий хвост через `Range:
  /// bytes=<уже скачано>-` (сервер за nginx отдаёт 206 Partial Content), а не
  /// перекачиваем всё заново. До [maxAttempts] попыток с нарастающей паузой.
  ///
  /// Кидает исключение, только если все попытки исчерпаны — тогда внешний
  /// catch подотрёт недокаченный файл.
  Future<void> _downloadWithResume(File file, UpdateInfo info) async {
    const maxAttempts = 6;
    final total = info.size;

    for (int attempt = 1; ; attempt++) {
      int existing = await file.exists() ? await file.length() : 0;
      // Уже всё на диске — sha256 проверится выше.
      ...
      try {
        ...
        // 206 — сервер понял Range и шлёт хвост (дописываем).
        // 200 — Range проигнорирован, отдаёт целиком (пишем с нуля).
        final partial = resp.statusCode == 206;
        ...
        final sink = file.openWrite(
          mode: (partial && existing > 0) ? FileMode.append : FileMode.write,
        );
        ...
        if (total <= 0 || got >= total) return; // докачали целиком
        // Стрим закрылся раньше времени без исключения — это и есть тот самый
        // обрыв; идём на следующую попытку дописать хвост.
        throw 'оборвано на $got/$total байт';
      } catch (e) {
        if (attempt >= maxAttempts) rethrow;
        debugPrint('Updater.download attempt $attempt failed: $e — повтор');
