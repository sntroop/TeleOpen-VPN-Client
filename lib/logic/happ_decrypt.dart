...
/// Достаёт happ://-ссылку из ссылки-обёртки.
///
/// Некоторые сервисы (ssconnect.app и подобные редиректоры) прячут happ-ссылку
/// в query-параметре в percent-кодировке, напр.:
///   https://ssconnect.app/link?url_ha=happ%3A%2F%2Fcrypt4%2FJ9QAEy...%3D
///
/// Если в [raw] (после percent-декодирования) есть `happ://` — возвращает
/// вырезанную happ-ссылку. Иначе возвращает [raw] без изменений.
String unwrapHappLink(String raw) {
  final t = raw.trim();
  if (t.contains('happ://')) return t; // уже «голая» happ-ссылка
  String decoded;
  try {
    decoded = Uri.decodeFull(t);
  } catch (_) {
    return t; // битая percent-кодировка — не трогаем
  }
  final i = decoded.indexOf('happ://');
  if (i < 0) return t;
  // base64-тело happ-ссылки не содержит & \n # пробел — на них и обрезаем.
  var end = decoded.length;
  for (final sep in const ['\n', '\r', ' ', '\t', '&', '#']) {
    final s = decoded.indexOf(sep, i);
    if (s >= 0 && s < end) end = s;
  }
  return decoded.substring(i, end);
}
