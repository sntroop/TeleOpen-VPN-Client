...
class IosField extends StatefulWidget {
  ...
  final bool enabled;          // false → поле затемнено и не принимает ввод
...
class _IosFieldState extends State<IosField> {
  ...
  Widget build(BuildContext context) {
    ...
    return Opacity(
      // Затемняем выключенное поле, чтобы было видно «пока недоступно».
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: (_focused && widget.enabled) ? c.blue.withValues(alpha: 0.5) : Colors.transparent,
          width: 1.5,
        ),
      ),
      child: Column(
