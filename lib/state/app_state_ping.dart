...
//   GET  — TcpPing.pingGet (прямой HTTP(S) GET до endpoint'а);
...
mixin AppStatePing on AppStateBase {
  ...
  Future<void> pingAll() async {
    ...
    void onResult(int i, int? ms) {
      ...
      // Пинг влияет на сортировку «по пингу» — инвалидируем кэш отрисовки.
      nodesRevision++;
    ...
    if (settings.pingMode == 'HTTP') {
      ...
      final targets =
          allNodes.map((n) => (host: n.address, port: n.port)).toList();
      await TcpPing.pingAll(targets, onResult, mode: settings.pingMode);
    }
  ...
  Future<int?> _pingNode(VpnNode n) async {
    switch (settings.pingMode) {
      ...
      case 'GET':
        return TcpPing.pingGet(n.address, n.port);
      case 'UDP':
