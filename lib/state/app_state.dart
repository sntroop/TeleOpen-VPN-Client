...
abstract class AppStateBase extends ChangeNotifier with WidgetsBindingObserver {
  ...
  Timer? _timer;
  // Момент подключения. Длительность считаем как (now - _connectStart), а не
  // инкрементом таймера: Timer.periodic засыпает в фоне и счётчик «замерзал»/
  // сбрасывался при долгом сворачивании. Timestamp же не зависит от тиков.
  ...
  // Готовность групп: ноды грузятся асинхронно из зашифрованного хранилища.
  // Холодный старт по deep-link/виджету может прилететь раньше — тогда ждём
  // это будущее, иначе _findNode вернёт null («сервер забыт»).
  final Completer<void> _groupsReady = Completer<void>();
  Future<void> get groupsReady => _groupsReady.future;
  ...
  // ═════ Сохранение нод (большие списки) ═════
  // Запись в файловое хранилище (NodeStore) дебаунсится: при пакетных мутациях
  // (импорт, обновление подписок) много вызовов _saveGroups коалесцируются в
  // одну запись. _saving защищает от наложения двух записей.
  Timer? _saveDebounce;
  bool _saving = false;
  bool _savePending = false;
  // Дебаунс синхронизации нативного виджета/тайла: exportProfiles строит конфиг
  // на каждую ноду — нельзя дёргать на каждую мелкую мутацию.
  Timer? _widgetSyncDebounce;

  /// Версия списка нод. Растёт при любом изменении состава/контента/избранного/
  /// пинга. Главный экран использует это как сигнал инвалидации кэша
  /// отсортированных/отфильтрованных списков (иначе пересортировка O(n log n)
  /// на каждый build при десятках тысяч нод).
  int nodesRevision = 0;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // При возврате из фона таймер мог проспать тики — пересчитываем
    // длительность подключения от timestamp и сразу обновляем UI.
    if (state == AppLifecycleState.resumed && status == VpnStatus.connected) {
      _recomputeConnectionDuration();
    }
  }

  /// Синхронизировать нативные виджет/тайл с текущим состоянием: лёгкий список
  /// серверов (для отрисовки) + готовые профили запуска (для старта без UI).
  /// Зовётся при изменении списка нод, настроек соединения или per-app.
  void syncWidgets() {
    // Дебаунс: при пакетных мутациях (импорт тысяч нод) не пересобираем профили
    // и список виджета на каждую мутацию — только один раз, когда всё устаканится.
    _widgetSyncDebounce?.cancel();
    _widgetSyncDebounce = Timer(const Duration(milliseconds: 500), _flushWidgets);
  }

  void _flushWidgets() {
    // ignore: discarded_futures
    WidgetBridge.exportServers(groups);
    // ignore: discarded_futures
    WidgetBridge.exportProfiles(groups, settings, perApp);
  }
  ...
  void _startTimer() {
    ...
    // Таймер нужен только чтобы тикать UI раз в секунду; само значение
    // пересчитываем от _connectStart, поэтому фоновая пауза тиков не теряет
    // время — при возврате показывается корректная длительность.
  ...
  /// Пересчитать длительность подключения от timestamp и уведомить UI.
  /// Вызывается по тику и при возврате приложения из фона (resume).
  void _recomputeConnectionDuration() {
    final start = _connectStart;
    if (start == null) return;
    connectionDuration = DateTime.now().difference(start);
    notifyListeners();
  }
  ...
  Future<void> connectByeDpi();                                // AppStateConnection
  ...
  void dispose() {
    ...
    _widgetSyncDebounce?.cancel();
    // Best-effort финальный флеш несохранённых нод.
    if (_savePending && !_saving) {
      // ignore: discarded_futures
      NodeStore.save(groups);
    }
    bridge.dispose();
    super.dispose();
  }
...
class AppState extends AppStateBase
  ...
  Future<void> _bootstrapSecure() async {
    ...
    // Туннель уже был жив на старте (см. _restoreNativeState), но activeNode
    // тогда ещё не был известен — группы грузятся асинхронно. Восстанавливаем
    // активную ноду сейчас, чтобы карточка показала имя сервера.
    if (status == VpnStatus.connected && activeNode == null) {
      final lastId = prefs.getString('last_active_node');
      if (lastId != null) activeNode = _findNode(lastId);
    }
    notifyListeners();
    reconfigureSubscriptionAutoUpdate(); // запустить таймер автообновления подписок
    // Авто-коннект только если туннель ещё НЕ поднят — иначе connect() порвёт
    // уже работающую сессию (он делает disconnect при status==connected).
    if (settings.autoConnect && status != VpnStatus.connected) await _autoConnect();
  }
  ...
  Future<void> _initBridge() async {
    await bridge.init(
      onStatus: (s) {
        ...
        if (status != newStatus) {
          ...
          // Обновляем нативный виджет/тайл актуальным статусом.
          // ignore: discarded_futures
          WidgetBridge.exportStatus(
            running: newStatus == VpnStatus.connected,
    ...
    // Холодный старт при живом туннеле: нативный сервис мог пережить
    // пересоздание Activity (приложение свернули/перезашли, VPN не отключали).
    // Опрашиваем нативное состояние и восстанавливаем «подключено», иначе UI
    // показывал бы «отключено» при работающем VPN. activeNode восстановит
    // _bootstrapSecure после загрузки групп.
    // ignore: discarded_futures
    _restoreNativeState();
  }

  Future<void> _restoreNativeState() async {
    final running = await bridge.isVpnRunning();
    if (running && status != VpnStatus.connected) {
      status = VpnStatus.connected;
      _sessionStart = DateTime.now();
      _startTimer();
      _startProbe();
      // ignore: discarded_futures
      WidgetBridge.exportStatus(running: true, remark: activeNode?.name ?? '');
      notifyListeners();
    }
  }
  ...
  void updateSettings(AppSettings s) {
    ...
    // Профили запуска виджета зависят от настроек (kill-switch, sniffing,
    // mux, resolveDestination) — пересобираем их.
    syncWidgets();
