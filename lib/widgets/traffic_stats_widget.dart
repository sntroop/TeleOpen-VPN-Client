// lib/widgets/traffic_stats_widget.dart
//
// Виджет статистики трафика для главного экрана.
// Показывает: текущая скорость ↓/↑, всего получено/отправлено, sparkline
// скорости за последние ~30 секунд.
//
// Использование:
//   const TrafficStatsWidget()
// Сам подписывается на AppState — рендерится только когда VPN активен.
...
class _TrafficStatsWidgetState extends State<TrafficStatsWidget> {
  // Последние 30 точек скорости — для sparkline. Заводим отдельно для rx/tx.
  ...
  void _ingest(VpnStats s) {
    ...
    while (_rxHistory.length > _historyLimit) {
      _rxHistory.removeFirst();
    }
    while (_txHistory.length > _historyLimit) {
  ...
  Widget build(BuildContext context) {
    ...
    if (state.status != VpnStatus.connected) {
      // сброс истории чтобы при следующем коннекте начать с чистого листа
...
String _formatDuration(int ms) {
  ...
  final sec = s % 60;
  if (h > 0) {
    return '$hч ${m.toString().padLeft(2, '0')}м ${sec.toString().padLeft(2, '0')}с';
  }
  return '${m.toString().padLeft(2, '0')}:${sec.toString().padLeft(2, '0')}';
}

/// Простой sparkline на CustomPainter — две линии (rx/tx).
...
class _SparklinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Общий масштаб для обоих рядов — чтобы было видно соотношение
    final maxVal = [
      ...rx, ...tx, 1,  // 1 чтобы не делить на 0
    ].reduce((a, b) => a > b ? a : b);
