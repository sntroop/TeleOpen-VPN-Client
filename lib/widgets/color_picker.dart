...
//
// Простой и приятный color picker:
// - 2D-палитра HSV (палец водит — меняется hue и saturation)
// - Слайдер яркости (value)
...
/// Показать color picker модалкой. Возвращает выбранный цвет или null если отменили.
...
class _SVPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // горизонтальный градиент: белый → прозрачный
    final whiteGrad = Paint()
      ..shader = LinearGradient(colors: [Colors.white, Colors.white.withValues(alpha: 0)]).createShader(rect);
...
class _AlphaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // градиент base → base с alpha=1
    canvas.drawRect(rect, Paint()..shader = LinearGradient(colors: [
      base.withValues(alpha: 0), base.withValues(alpha: 1),
    ]).createShader(rect));
