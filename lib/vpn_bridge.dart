...
class VpnBridge {
  ...
  Future<bool> start({
    ...
    bool xrayTunMode = false, // NEW
    bool keepDeviceAwake = false, // NEW
    int memoryLimitMB = 100, // NEW
    bool proxyOnlyMode = false, // NEW
    String socksAuthUser = '', // NEW
    String socksAuthPass = '', // NEW
  }) async {
    try {
      await _method.invokeMethod('startVpn', {
        ...
        'xrayTunMode': xrayTunMode,
        'keepDeviceAwake': keepDeviceAwake,
        'memoryLimitMB': memoryLimitMB,
        'proxyOnlyMode': proxyOnlyMode,
        'socksAuthUser': socksAuthUser,
        'socksAuthPass': socksAuthPass,
      });
      return true;
  ...
  /// Запустить режим обхода DPI: локальный ciadpi (SOCKS5 с десинхронизацией)
  /// + tun2socks, БЕЗ VPN-сервера — трафик идёт напрямую в интернет.
  /// [args] — аргументы командной строки ciadpi (см. buildByeDpiArgs).
  Future<bool> startByeDpi({
    required List<String> args,
    int socks5Port = 10800,
    String remark = 'ByeDPI',
    bool perAppEnabled = false,
    List<String> allowedPackages = const [],
    bool killSwitch = false,
    bool keepDeviceAwake = false, // NEW
    ...
    try {
      await _method.invokeMethod('startByeDpi', {
        'args': args,
        'socks5Port': socks5Port,
        'remark': remark,
        'perAppEnabled': perAppEnabled,
      ...
      debugPrint('=== ByeDPI start error: ${e.message}');
      return false;
    } catch (e) {
      debugPrint('=== ByeDPI start error: $e');
      return false;
    }
  }
  ...
  /// Жив ли нативный VPN-сервис прямо сейчас. Нужно при холодном старте UI:
  /// foreground-сервис мог пережить пересоздание Activity (свернули/перезашли),
  /// и приложение должно восстановить состояние «подключено», а не показывать
  /// «отключено» при работающем туннеле.
  Future<bool> isVpnRunning() async {
    try {
      final res = await _method.invokeMethod<dynamic>('isVpnRunning');
      if (res is Map) return (res['running'] as bool?) ?? false;
      return false;
    } catch (e) {
      debugPrint('=== isVpnRunning error: $e');
      return false;
    }
  }
  ...
  /// Сменить иконку приложения в лаунчере (Android: activity-alias).
  /// [variant] — ключ варианта ('default', 'blue', 'green', ...), совпадает
  /// с ICON_ALIASES в MainActivity.kt. На платформах без поддержки тихо
  /// игнорируется. Возвращает true при успехе.
  Future<bool> setAppIcon(String variant) async {
    try {
      final ok =
          await _method.invokeMethod<bool>('setAppIcon', {'variant': variant});
      return ok ?? false;
    } on PlatformException catch (e) {
      debugPrint('=== setAppIcon error: ${e.message}');
      return false;
    } catch (e) {
      debugPrint('=== setAppIcon error: $e');
      return false;
    }
  }
