          ...
          child: IosCard(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(
                          state.settings.bdpiModeEnabled
                              ? 'Обход DPI'
                              : state.activeNode?.name ?? 'Сервер не выбран',
                           style: t.textStyles.body, overflow: TextOverflow.ellipsis),
                      if (state.settings.bdpiModeEnabled)
                        Text(
                          'Десинхронизация без сервера',
              ...
              IosButton(
                ...
                // В режиме обхода DPI сервер не нужен — кнопка активна без ноды.
                onPressed: (!state.settings.bdpiModeEnabled &&
                            state.activeNode == null && !isConnected)
                    ? null
                    : () {
                        if (isConnected) {
                          state.disconnect();
                        } else if (state.settings.bdpiModeEnabled) {
                          state.connectByeDpi();
                        } else if (state.activeNode != null) {
