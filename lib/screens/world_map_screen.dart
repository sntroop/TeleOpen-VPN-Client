...
class _ServerCluster {
  ...
  /// Округляем координаты до 0.5° (~50 км) — серверы в одном городе попадают в один кластер.
...
class _WorldMapScreenState extends State<WorldMapScreen> with TickerProviderStateMixin {
  ...
  /// Web Mercator projection (как Google/OSM): растягивает северные широты.
...
// ════════════════════════════════════════════════════════════════════════════
// Маркер (с опциональным бейджем «2», «3»…)
// ════════════════════════════════════════════════════════════════════════════
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      ...
      children: [
        ...
        // Бейдж с количеством — рендерится в child-coords, поэтому компенсируем зум
        if (badgeCount != null)
          Center(
...
// ════════════════════════════════════════════════════════════════════════════
// Bottom-sheet выбора сервера из кластера с несколькими серверами
// ════════════════════════════════════════════════════════════════════════════
