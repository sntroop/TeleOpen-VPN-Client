// lib/screens/dns_leak_test_screen.dart
//
// Тест утечки DNS:
//   - резолвит несколько канареечных доменов через системный резолвер,
//   - собирает уникальные резолверы из заголовков ответа (whoami-сервисы
//     вроде resolver.dnscrypt.info, whoami.akamai.net и т.п.),
//   - выводит список резолверов и их геолокацию.
//
// Если среди резолверов есть провайдер пользователя (а не апстрим
// VPN/прокси) — это утечка.
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            const SliverToBoxAdapter(child: _ScreenHeader(title: 'Тест утечки DNS')),
