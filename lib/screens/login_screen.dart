...
//
// Экран входа через Telegram (TeleOpenLoginBot).
// Flow:
//   1) POST /auth/init → token
//   2) Открываем https://t.me/TeleOpenLoginBot?start=<token>
//   3) Каждые 2 сек: GET /auth/poll/<token>
//   4) Когда status="ok" — сохраняем TgUser и закрываем экран.
...
class _LoginScreenState extends State<LoginScreen> with WidgetsBindingObserver {
  ...
  Timer? _pollTimer;
  int _secondsLeft = 600; // 10 минут на авторизацию
  Timer? _countdown;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      // Вернулись из Telegram → форсим перерисовку (на Android иначе чёрный экран),
      // и сразу делаем пинг сервера — может уже залогинились.
      if (mounted) setState(() {});
  ...
  Future<void> _poll() async {
    ...
    try {
      ...
      _countdown?.cancel();
      if (!mounted) return;
      // Сохраняем JWT в MarketApi и в AppState
      ...
      // прочие ошибки игнорируем — продолжим polling
    } catch (_) {
      // network — продолжаем
    }
  }
