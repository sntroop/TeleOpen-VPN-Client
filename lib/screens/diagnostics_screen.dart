...
class _NodePickerState extends State<_NodePicker> {
  // id групп, которые сейчас развёрнуты. По умолчанию все свёрнуты, чтобы
  // длинный список серверов не вываливался разом.
  ...
  Widget build(BuildContext context) {
    ...
    final groups = state.groups.where((g) => g.nodes.isNotEmpty).toList();
    ...
    return ListView(
      ...
      children: [
        ...
        ...groups.map((g) {
          final isOpen = _expanded.contains(g.id);
          return Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              // ── тапабельный заголовок группы ───────────────────────────────
              GestureDetector(
                behavior: HitTestBehavior.opaque,
                onTap: () => setState(() {
                  if (isOpen) {
              ...
              // ── список серверов (только если развёрнут) ────────────────────
              if (isOpen)
                IosListSection(
                  children: g.nodes
                      .map((n) => IosListTile(
